...
def darcy_fluxes(GRID, dt):
    ...
    for Idx in range(0, GRID.number_nodes - 1):
        ...
        if q_lim > 0:
            D[Idx] = min(GRID.get_node_liquid_water_content(Idx), q_lim * (1 - np.exp(- q0 / q_lim * dt)))
        else:
            D[Idx] = 0
...
"""
    ...
    return min(1,max(0,((lwc - residual_water_content) / \
              (((snow_ice_threshold - GRID.get_node_ice_fraction(Idx) * ice_density) / water_density) - residual_water_content))))
