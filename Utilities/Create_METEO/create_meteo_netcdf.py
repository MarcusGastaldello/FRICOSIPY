...
def create_meteo_input(csv_file, meteo_file, start_date = None, end_date = None):
    ...
    T2 = np.asarray(df['T2'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'T2_LAPSE' in df.columns:
        ...
        T2_LAPSE = np.asarray(df['T2_LAPSE'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    PRES = np.asarray(df['PRES'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    U2 = np.asarray(df['U2'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    RH2 = np.asarray(df['RH2'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'SWin' in df.columns:
        ...
        SWin = np.asarray(df['SWin'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'LWin' in df.columns:
        ...
        LWin = np.asarray(df['LWin'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'N' in df.columns:
        ...
        N = np.asarray(df['N'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'ACC_ANOMALY' in df.columns:
        ...
        ACC_ANOMALY = np.asarray(df['ACC_ANOMALY'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
    ...
    if 'SUB_ANOMALY' in df.columns:
        ...
        SUB_ANOMALY = np.asarray(df['SUB_ANOMALY'].apply(pd.to_numeric, errors='coerce'), dtype = np.float64)
