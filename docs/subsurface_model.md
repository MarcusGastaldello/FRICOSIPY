...
<small>where $K$ is the thermal diffusivity $(m^2$ $s^{-1})$, $k$ is the thermal conductivity $(W$ $m^{-1}$ $K^{-1})$, $\rho$ is the density and $c_p$ is the specific heat capacity under constant pressure $(J$ $kg^{-1}$ $K^{-1})$  </small>
