...
The temporal range and spatial extent of the simulation must also be specified by the user. This is accomplished by providing datetime values [yyyy-mm-dd hh:mm] indicating the starting and ending timestamp (which must be within the datetime range of the METEO file) and a bounding box of easting $(x)$ and northing $(y)$ values to demarcate the spatial extent. Regarding the latter, by default the user would set ´spatial_subset´= False´ and [x_min, x_max, y_min, y_max] = None´ to use the entirity of the spatial domain of the STATIC file.
