...
<div style="border:1px solid #ccc; padding:10px; background:#f9f9f9;">
$$
SW_{in} = I_{TOA} \: \Delta ( $x$ , $y$ ) \: \tau_{rg} \: \tau_{w} \: \tau_{aerosols} \: \tau_{clouds} 
$$
</div>
<small>where $SW_{in}$ is the incoming shortwave radiation flux, $I_{TOA}$ is the unattenuated Top-of-Atmosphere (TOA) irradiance on a surface normal to the incident radiation, $\Lamda$ ( $x$ , $y$ ) is a correction factor for an inclined surface and $\tau_{rg}$, $\tau_{w}$, $\tau_{aerosols}$ & $\tau_{clouds}$ are the coefficients of atmospheric transmissivity for *Rayleigh* scattering and gaseous absorption, water absorption, aerosols and cloud cover respectively .</small>
