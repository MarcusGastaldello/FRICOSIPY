...
def update_albedo(GRID,surface_temperature,albedo_snow):
    """ This methods updates the albedo """
    albedo_allowed = ['bucket','Marchenko17']
    if albedo_method == 'Oerlemans98':
        albedo, albedo_snow = method_Oerlemans(GRID)
    elif albedo_method == 'Bougamont05':
        albedo, albedo_snow = method_Bougamont(GRID,surface_temperature,albedo_snow)
    else:
...
def method_Bougamont(GRID,surface_temperature,albedo_snow):
    ...
    # Get time difference between last snowfall and now:
    hours_since_snowfall = (fresh_snow_timestamp) / 3600.0
    days_since_snowfall = (fresh_snow_timestamp) / 86400.0
    ...
    albedo_snow = albedo_snow - (albedo_snow - albedo_firn) / t_star * dt_days #EBFM
    #albedo_snow = albedo_firn + ((albedo_fresh_snow - albedo_firn) * np.exp(-days_since_snowfall/t_star))
